...
export const useMatchConnection = () => {
  ...
  useEffect(() => {
    if (!roomId || !user) {
      ...
      setState(prev => ({ ...prev, error: 'Room ID or user not found', isLoading: false }));
      return;
    }
    ...
    const subscribeToRoom = async () => {
      try {
        ...
        if (!room.host || (room.guest && room.guest.uid !== user.uid && room.host.uid !== user.uid)) {
        ...
        const currentPlayer = room.host.uid === user.uid ? room.host : room.guest || null;
        const otherPlayer = room.host.uid === user.uid ? room.guest || null : room.host;
        ...
        const unsubscribeRoom = roomService.subscribeToRoom(roomId, (updatedRoom) => {
          if (updatedRoom) {
            console.log('🔄 Hook: Actualización de sala recibida:', updatedRoom.roomCode);
            const updatedCurrentPlayer = updatedRoom.host.uid === user.uid ? updatedRoom.host : updatedRoom.guest || null;
            const updatedOtherPlayer = updatedRoom.host.uid === user.uid ? updatedRoom.guest || null : updatedRoom.host;
        ...
        const unsubscribeMessages = roomService.subscribeToMessages(roomId, (messages) => {
          console.log('📨 Hook: Mensajes recibidos:', messages.length, messages);
          setState(prev => ({
            ...prev,
            messages
          }));
