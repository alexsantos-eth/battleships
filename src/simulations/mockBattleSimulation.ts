...
export function createMockBattleWithPlayerWin(
  seed: number = 12345
): MockBattleResult {
  ...
  const instructions: BattleInstruction[] = [
    ...
    { type: "fire_shot", data: { position: { x: 6, y: 5 } } },
...
export function createMockBattleWithEnemyWin(
  seed: number = 67890
): MockBattleResult {
  const config: BattleConfig = {
    seed,
...
export function createRandomBattleSimulation(
  seed: number = 99999,
  maxShots: number = 15
): MockBattleResult {
  const config: BattleConfig = {
