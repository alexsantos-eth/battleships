...
export function runQuickGameSimulation() {
  ...
  // Agregar pocos barcos pequeños para que el juego termine rápido
  const playerShips = [
    { position: { x: 0, y: 0 }, variant: "small" as const, orientation: "horizontal" as const },
    { position: { x: 5, y: 5 }, variant: "small" as const, orientation: "vertical" as const },
  ];
  
  const enemyShips = [
    { position: { x: 1, y: 1 }, variant: "small" as const, orientation: "horizontal" as const },
    { position: { x: 6, y: 6 }, variant: "small" as const, orientation: "vertical" as const },
  ];
  ...
  // Simular disparos hasta que termine el juego
  while (!game.isGameOver()) {
    console.log(`\n--- Turno ${turn} ---`);
    console.log("Turno actual:", game.getCurrentTurn());
    
    if (game.isPlayerTurn()) {
      // Disparos del jugador contra el enemigo
      ...
      for (const ship of enemyShips) {
        ...
        for (const pos of positions) {
          if (!game.isPositionShot("enemy", pos)) {
            ...
            console.log(`Jugador dispara en (${pos.x}, ${pos.y}) - ${shot.hit ? "¡HIT!" : "Miss"}`);
      ...
      // Disparos del enemigo contra el jugador
      ...
      for (const ship of playerShips) {
        ...
        for (const pos of positions) {
          if (!game.isPositionShot("player", pos)) {
            ...
            console.log(`Enemigo dispara en (${pos.x}, ${pos.y}) - ${shot.hit ? "¡HIT!" : "Miss"}`);
