...
export const Navigation: React.FC<NavigationProps> = ({ className = '' }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 bg-black/30 backdrop-blur-lg border-b border-white/20 ${className}`}>
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-12">
          <Link to="/" className="flex items-center space-x-2 group">
            <span className="text-xl group-hover:scale-110 transition-transform duration-200">⚓</span>
            <span className="text-lg font-bold text-white group-hover:text-blue-300 transition-colors duration-200">Armada.io</span>
          </Link>
          ...
          <div className="flex items-center space-x-2">
            <div className="relative">
              ...
              >
                ...
                <span className="hidden sm:block text-sm">{profile?.displayName || 'User'}</span>
