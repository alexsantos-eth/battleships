...
export const UserStats: React.FC<UserStatsProps> = ({ className = '' }) => {
  ...
  const accuracy = stats.totalShots > 0 ? ((stats.totalHits / stats.totalShots) * 100).toFixed(1) : '0.0';

  return (
    <div className={`bg-white/10 backdrop-blur-lg rounded-lg p-4 shadow-lg border border-white/20 ${className}`}>
