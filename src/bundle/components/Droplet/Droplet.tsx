...
import type { DropletProps } from "./Droplet.types";
...
export const Droplet: React.FC<DropletProps> = ({ position, dir, onDone }) => {
  ...
  const { pos, opacity } = useSpring({
    from: {
      pos: [animation.from.x, animation.from.y, animation.from.z] as [
        number,
        number,
        number
      ],
    ...
    to: {
      pos: [animation.to.x, animation.to.y, animation.to.z] as [
        number,
        number,
        number
      ],
