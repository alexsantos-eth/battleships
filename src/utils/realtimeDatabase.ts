...
export class RealtimeDatabase {
  ...
  private buildQuery(dbRef: DatabaseReference | Query, constraints: RealtimeQueryConstraint[]): Query {
    ...
    for (const constraintItem of constraints) {
      ...
      if (constraintItem.value !== undefined) {
        dbQuery = query(dbQuery, equalTo(constraintItem.value as string | number | boolean));
      }

      if (constraintItem.startAt !== undefined) {
        dbQuery = query(dbQuery, startAt(constraintItem.startAt as string | number | boolean));
      }

      if (constraintItem.endAt !== undefined) {
        dbQuery = query(dbQuery, endAt(constraintItem.endAt as string | number | boolean));
      }
  ...
  async queryDocumentsPaginated<T>(
    ...
    try {
      ...
      const resultDocuments = hasMore ? documents.slice(0, pageSize) : documents;
      const lastKey = resultDocuments.length > 0 
        ? (resultDocuments[resultDocuments.length - 1] as Record<string, unknown>).id as string
        : null;
  ...
  async loadMoreDocuments<T>(
    ...
    try {
      ...
      const hasMore = documents.length > pageSize;
      const resultDocuments = hasMore ? documents.slice(0, pageSize) : documents;
      const newLastKey = resultDocuments.length > 0 
        ? (resultDocuments[resultDocuments.length - 1] as Record<string, unknown>).id as string
        : null;
...
export const dbUtils = {
  ...
  getDocument: <T>(
    path: string
  ) => realtimeDatabase.getDocument<T>(path),
  ...
  deleteDocument: (
    path: string
  ) => realtimeDatabase.deleteDocument(path),
  ...
  subscribeToCollection: <T>(
    ...
    constraints?: RealtimeQueryConstraint[],
    options?: RealtimeOptions
  ) => realtimeDatabase.subscribeToCollection(path, callback, constraints, options),
  ...
  documentExists: (
    path: string
  ) => realtimeDatabase.documentExists(path),
