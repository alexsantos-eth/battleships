...
import type { GameRoom, RoomPlayer, GameConfig } from '@/types/game';
...
export const useRoom = (roomId?: string) => {
  ...
  const createRoom = useCallback(async (displayName: string): Promise<GameRoom> => {
    if (!user) {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error al crear la sala';
  ...
  const joinRoom = useCallback(async (roomCode: string, displayName: string): Promise<GameRoom> => {
    if (!user) {
      throw new Error('Usuario no autenticado');
    }
  ...
  const setPlayerReady = useCallback(async (isReady: boolean): Promise<void> => {
    if (!room || !user) {
  ...
  const startGame = useCallback(async (gameConfig: GameConfig): Promise<void> => {
    if (!room || !user) {
      throw new Error('Sala o usuario no disponible');
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error al iniciar el juego';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  const leaveRoom = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error al salir de la sala';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
